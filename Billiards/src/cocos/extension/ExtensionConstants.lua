...
--参数：  
cc.EDITBOX_INPUT_MODE_ANY = 0                                --用户可以输入任何文字  
cc.EDITBOX_INPUT_MODE_EMAILADDR = 1                          --用户可以输入一个电子邮件地址  
cc.EDITBOX_INPUT_MODE_NUMERIC = 2                            --用户被允许进入一个整数值  
cc.EDITBOX_INPUT_MODE_PHONENUMBER = 3                        --用户可以输入一个电话号码  
cc.EDITBOX_INPUT_MODE_URL = 4                                --用户可以输入一个网址 
cc.EDITBOX_INPUT_MODE_DECIMAL = 5                            --用户被允许进入一个真正的数值  
cc.EDITBOX_INPUT_MODE_SINGLELINE = 6                         --用户可以输入任何文字，换行除外


--输入的字符格式  
--editBox:setInputFlag(inputFlag)  
--参数：  
cc.EDITBOX_INPUT_FLAG_PASSWORD = 0                            --密码
cc.EDITBOX_INPUT_FLAG_SENSITIVE = 1                           --表示输入的文本是敏感数据  
cc.EDITBOX_INPUT_FLAG_INITIAL_CAPS_WORD = 2                   --每个单词的首字母应该大写
cc.EDITBOX_INPUT_FLAG_INITIAL_CAPS_SENTENCE = 3               --每个句子的首字母应该大写
cc.EDITBOX_INPUT_FLAG_INITIAL_CAPS_ALL_CHARACTERS = 4         --自动大写的所有字符


--设置返回类型    就是键盘右下角那个键是 
--return 、Done 、Send 、Search 、Go等字样   （抄来的，不太了解）  
