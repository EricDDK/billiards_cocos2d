...
function AppBase:enterScene(sceneName, transition, time, more)
    if loadingMgr and loadingMgr.clearGameCahe then
        loadingMgr:clearGameCahe()
    end
    if sceneName == "GameHallScene" then
        print("AppBase EnterScene Hall Scene , stack is ",debug.traceback())
    end
    tool:resetStateInReplaceScene()

    if player:getIsPassthroughParamStart() then
        if player:getPlayerStartParam().gameType == G_GameType.DZPOKER then
           cc.Director:getInstance():replaceScene(require("hallcenter.views.GameRoomScene").new(G_Game_NumType.DZPOKER)) 
           amgr.playMusic("hall_res/background.mp3", true)
            return
        end
    end
